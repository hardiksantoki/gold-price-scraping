...
class Domain1Spider(scrapy.Spider):
    ...
    def parse(self, response):
        price = response.css('#maincontent table.prices-tier.items tbody tr td:nth-child(2) span strong.price-formatted::text').get()
        id = response.meta['id']
...
class SdBullionSpider(scrapy.Spider):
    custom_settings = {
    ...
    # start_urls = ["https://sdbullion.com/"]
    ...
    def start_requests(self):
        ...
        cursor = conn.cursor()
        cursor.execute("SELECT id, link, product_name, dealer FROM findbull WHERE dealer LIKE '%SD Bullion%'")  # Assuming you have an ID column
